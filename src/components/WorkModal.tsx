      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-4 md:inset-8 lg:inset-16 z-50 bg-black/30 rounded-2xl overflow-hidden shadow-2xl">
            <div className="relative h-full flex flex-col md:flex-row">
              ...
              <div className="flex-1 p-8 md:p-12 flex flex-col justify-center text-white">
                ...
                {/* Title */}
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">{work.title}</h2>

                {/* Description */}
                <p className="text-lg md:text-xl text-white/80 leading-relaxed mb-8">{work.description}</p>

                {/* Additional Info */}
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-white rounded-full" />
                    <span className="text-white/70">Project ID: #{work.id.toString().padStart(3, "0")}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-white rounded-full" />
                    <span className="text-white/70">Category: {work.category}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-white rounded-full" />
                    <span className="text-white/70">Year: {work.year}</span>
                  </div>
                </div>
              </div>
            </div>
